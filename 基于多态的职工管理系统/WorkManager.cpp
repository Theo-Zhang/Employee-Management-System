...
//添加职工
 void WorkerManager::Add_Emp() {
	 ...
	 if (addNum > 0) {
		 ...
		 if (this->m_EmpArray != NULL) {
			 for (int i = 0; i < this->m_EmpNum; i++) {
				 newSpace[i] = this->m_EmpArray[i]; //把原来的数据都拷贝下来
			 }
		 }
 ...
 int WorkerManager::get_EmpNum() {
	 ...
	 while (ifs >> id && ifs >> name && ifs >> dId) //ifs 读数据，读完一行，++
	 {	
		 //统计人数变量
		 num++;
	 }
 ...
 void WorkerManager::Mod_Emp() {
	 ...
	 if (this->m_FileIsEmpty) {
		 ...
		 if (ret != -1) {
			 ...
			 delete this->m_EmpArray[ret]; //把原来的堆区数据释放干净
 ...
 //判断职工是否存在 如果存在返回职工所在数组中的位置，如果不存在，返回-1
